...
/**
 * Follow an SDG from the hub page.
 * Inserts a `user_followed_sdgs` row; silently ignores duplicates.
 */
export async function followSdgAction(sdgId: number): Promise<{ ok: boolean; error?: string }> {
  ...
  revalidatePath('/[locale]/(public)/sdg/[code]', 'page');
  return { ok: true };
}

/**
 * Unfollow an SDG from the hub page.
 * Deletes the `user_followed_sdgs` row.
 */
