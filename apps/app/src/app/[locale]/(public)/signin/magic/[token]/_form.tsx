...
import { magicLinkConsumeAction } from '../../../_actions/auth.ts';

interface MagicLinkConsumeFormProps {
  token: string;
  locale: string;
}

export function MagicLinkConsumeForm({ token, locale: _locale }: MagicLinkConsumeFormProps) {
  const t = useTranslations('Auth');
  const [state, formAction, isPending] = useActionState(magicLinkConsumeAction, {
    ok: false,
    errors: {},
  });
